...
ALTER TABLE public.tasks ADD COLUMN team_members JSONB DEFAULT '[]';
ALTER TABLE public.tasks ADD COLUMN required_skills JSONB DEFAULT '[]';
ALTER TABLE public.tasks ADD COLUMN task_permissions JSONB DEFAULT '{}'; -- Role-based permissions
...
INSERT INTO public.materials (name, description, unit, density, carbon_emission_factor, material_category, cost_per_unit, created_by, is_public) VALUES
('Concrete C30/37', 'High-strength concrete for structural applications', 'kg', 2400, 0.12, 'Concrete', 0.08, (SELECT auth.uid()), true),
('Steel Rebar', 'Reinforcement steel bars for concrete structures', 'kg', 7850, 2.3, 'Steel', 0.85, (SELECT auth.uid()), true),
('Brick', 'Standard clay bricks for masonry work', 'piece', 1800, 0.24, 'Masonry', 0.35, (SELECT auth.uid()), true),
('Timber - Pine', 'Softwood timber for construction', 'm3', 500, -0.9, 'Timber', 350, (SELECT auth.uid()), true),
('Insulation - Mineral Wool', 'Thermal insulation material', 'm2', 50, 1.2, 'Insulation', 12, (SELECT auth.uid()), true);
